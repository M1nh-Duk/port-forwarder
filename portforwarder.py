...
class Filter:
	...
	def filter_response(data, id):
		...
		#print(decoded_data)
		if decode_http.isHTTP(decoded_data) and "Content-Encoding: gzip" in decoded_data:
			decoded_data = decode_http.httpDecodeContent(decoded_data)
			#print(decoded_data)
		  
		#  Filter
		for re_rule in regex_rules:
			try:
				if re.search(re_rule['sign'], decoded_data):
		...
		for string_rule in string_rules:
			if string_rule['sign'] in decoded_data:
